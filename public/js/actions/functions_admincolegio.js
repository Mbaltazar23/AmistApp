...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formAdmin.onsubmit = function (e) {
        ...
        if (strNombre == "" || txtRut == "" || txtCorreo == "" || txtTelefono == "") {
            ...
            axios.post("/adminsColleges/setAdmin", formData).then(function (response) {
    ...
    formColegioA.onsubmit = function (e) {
        ...
        if (listColegios == "") {
            ...
            axios.post("/adminsColleges/setCollege", formData).then(function (response) {
                if (response.status) {
                    tableAdmins.api().ajax.reload();
...
function fntViewInfo(idAdmin) {
    axios.get(`/adminsColleges/getAdmin/${idAdmin}`).then(function (response) {
        if (response.data.status) {
            let estado = response.data.data.status == 1 ? '<span class="badge badge-success">Activo</span>' : '<span class="badge badge-dark">Vinculado</span>';
            ...
            document.querySelector("#celEmail").innerHTML = response.data.data.email;
...
function fntEditInfo(element, idAdmin) {
    ...
    axios.get(`/adminsColleges/getAdmin/${idAdmin}`).then(function (response) {
        if (response.data.status) {
            document.querySelector("#idAdmin").value = response.data.data.id;
            document.querySelector("#txtDni").value = response.data.data.dni;
            document.querySelector("#txtNombre").value = response.data.data.nombre.toString().toLowerCase();
            document.querySelector("#txtEmail").value = response.data.data.email.toString().toLowerCase();
            document.querySelector("#txtTelefono").value = response.data.data.telefono;
            document.querySelector("#txtDireccion").value = response.data.data.direccion.toString().toLowerCase();
            // Mostrar campo de contraseña sin valor
...
function fntDelInfo(idadmin) {
    swal({
        ...
        if (isClosed) {
            let status = 0;
            axios.post(`/adminsColleges/status/${idadmin}`, {status: status}).then((response) => {
                if (response.data.status) {
...
function openModal() {
    ...
    document.querySelector("#titleModal").innerHTML = "Nuevo Administrador de Colegio";
...
function fntSchoolA(idAdmin) {
    ...
    axios.get(`/adminsColleges/getAdmin/${idAdmin}`).then(function (response) {
        if (response.data.status) {
            document.querySelector("#idAdminC").value = response.data.data.id;
            document.querySelector("#idVinCol").value = "";
            document.querySelector("#txtDniA").value = response.data.data.dni;
            document.querySelector("#txtNombreA").value = response.data.data.nombre;
...
function fntSchoolU(idAdmin) {
    ...
    axios.get(`/adminsColleges/getAdmin/${idAdmin}`).then(function (response) {
        if (response.data.status) {
            document.querySelector("#idAdminC").value = response.data.data.id;
            document.querySelector("#idVinCol").value = response.data.data.idColegioUser;
            document.querySelector("#txtDniA").value = response.data.data.dni;
            document.querySelector("#txtNombreA").value = response.data.data.nombre;
            document.querySelector("#listColegios").value = response.data.data.colegio_id;
            $("#modalFormColegiosAd").modal("show");
        }
    }).catch(function (error) {
        console.log(error);
...
function fntDelSchool(idadmin) {
    swal({
        ...
        if (isClosed) {
            axios.post(`/adminsColleges/delCollege/${idadmin}`).then((response) => {
                if (response.data.status) {
                    swal("Exito !!", response.data.msg, "success");
                    tableAdmins.api().ajax.reload();
                } else {
...
function fntColegios() {
    if (document.querySelector("#listColegios")) {
        axios.post("/colleges/select").then((response) => {
...
function generarReporte() {
    axios.post("/adminsColleges/report").then(function (response) {
        var fecha = new Date();
        ...
        for (let i = 0; i < adminsColegio.length; i++) {
            if (adminsColegio[i].status == 1) {
                ...
                adminsColegio[i].address != "" ? adminsColegio[i].address : "No tiene una dirección registrada",
                estado,
            ];
        }
