...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formColegio.onsubmit = function (e) {
        ...
        let txtStockAlumns = parseInt(document.querySelector("#txtStockAlumns").value);
        // Convertir a número entero
        // var regexClave = new RegExp("^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$");
        var regulTele = /^(\+?56)?(\s?)(0?9)(\s?)[9876543]\d{7}$/;
        if (strNombre == "" || txtRut == "" || txtTelefono == "" || isNaN(txtStockAlumns) || txtStockAlumns < 0) {
            ...
            axios.post("/colleges/setCollege", formData).then(function (response) {
...
function fntViewInfo(idColegio) {
    axios.get(`/colleges/getCollege/${idColegio}`).then(function (response) {
        if (response.data.status) {
            let estado = response.data.data.status == 1 ? '<span class="badge badge-success">En espera</span>' : '<span class="badge badge-dark">Vinculado</span>';

            let direccion = response.data.data.direccion != "" ? response.data.data.direccion : "No se tiene una direccion registrada";
            document.querySelector("#celRut").innerHTML = response.data.data.rut;
            ...
            document.querySelector("#celAlumnosStock").innerHTML = response.data.data.stock_alumnos;
...
function fntEditInfo(element, idColegio) {
    ...
    axios.get(`/colleges/getCollege/${idColegio}`).then(function (response) {
        if (response.data.status) {
            document.querySelector("#idColegio").value = response.data.data.id;
            document.querySelector("#txtRut").value = response.data.data.rut;
            ...
            document.querySelector("#txtStockAlumns").value = response.data.data.stock_alumnos;
...
function fntDelInfo(idcolegio) {
    swal({
        ...
        if (isClosed) {
            let status = 0;
            axios.post(`/colleges/status/${idcolegio}`, {status: status}).then((response) => {
                if (response.data.status) {
...
function fntActivateInfo(idcolegio) {
    swal({
        ...
        if (isClosed) {
            let status = 1;
            axios.post(`/colleges/status/${idcategoria}`, {status: status}).then((response) => {
                if (response.data.status) {
...
function generarReporte() {
    axios.post("/colleges/report").then(function (response) {
        var fecha = new Date();
        ...
        for (let i = 0; i < colegios.length; i++) {
            if (colegios[i].status == 1) {
                ...
                colegios[i].name,
                colegios[i].phone,
                colegios[i].address != "" ? colegios[i].address : "No tiene una dirección registrada",
                colegios[i].stock_alumns + " Alumnos",
                estado,
            ];
        }
