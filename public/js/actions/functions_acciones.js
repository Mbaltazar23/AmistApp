...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formAccion.onsubmit = function (e) {
        ...
        if (strNombre == "" || strPuntos == "" || strTipo == "") {
            swal("Atención", "Debe ingresar un nombre y su puntaje..", "error");
            return false;
        } else {
            let formData = new FormData(formAccion);
            axios.post("/actions/setAction", formData).then(function (response) {
...
function fntViewInfo(nro, idaccion) {
    axios.get(`/actions/getAction/${idaccion}`).then(function (response) {
        if (response.data.status) {
            ...
            document.querySelector("#celPuntaje").innerHTML = response.data.data.puntos;
            ...
            document.querySelector("#celTipo").innerHTML = "Para " + response.data.data.tipo;
...
function fntEditInfo(element, idaccion) {
    ...
    axios.get(`/actions/getAction/${idaccion}`).then(function (response) {
        if (response.data.status) {
            document.querySelector("#idAccion").value = response.data.data.id;
            document.querySelector("#txtNombre").value = String(response.data.data.nombre).toLowerCase();
            document.querySelector("#txtVisible").value = response.data.data.tipo;
            document.querySelector("#txtPuntaje").value = response.data.data.puntos;
...
function fntDelInfo(idaccion) {
    swal({
        ...
        if (isClosed) {
            let status = 0;
            axios.post(`/actions/status/${idaccion}`, {status: status}).then((response) => {
                if (response.data.status) {
...
function generarReporte() {
    axios.post("/actions/report").then(function (response) {
        var fecha = new Date();
        ...
        var columns = [
            ...
            "PUNTOS",
            "FECHA",
            "COLEGIOS", // Agregar columna para la cantidad de veces por colegio
            "USADO", // Agregar columna para los colegios
        ];
        var data = [];
        ...
        for (let i = 0; i < acciones.length; i++) {
            ...
            let colegios = acciones[i].colegios.split(", "); // Separar los nombres de los colegios
            let cantidadColegios = acciones[i].cantidad_colegios; // Obtener la cantidad de veces por colegio
            let colegiosData = [];
            colegios.forEach(function (colegio) {
                if (cantidadColegios[colegio]) {
                    colegiosData.push(colegio + ": " + cantidadColegios[colegio]);
                }
            });
            data[i] = [
                i + 1,
                ...
                acciones[i].tipo,
                acciones[i].puntos,
                acciones[i].fecha + "" + acciones[i].hora,
                acciones[i].colegios,
                colegiosData.join("\n"), // Agregar los nombres de los colegios con la cantidad de veces
            ];
        }
