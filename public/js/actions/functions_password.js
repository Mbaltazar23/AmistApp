$(document).ready(function () {
    if (document.querySelector("#formRecover")) {
        ...
        formRecover.onsubmit = function (e) {
            ...
            if (txtEmail == "") {
                swal("Por favor", "Escriba su correo para identificarlo.", "error");
                return false;
                ...
                axios.post("/login/getEmail", {email: txtEmail}).then(function (response) {
    ...
    if (document.querySelector("#formResetPass")) {
        ...
        formResetPass.onsubmit = function (e) {
            ...
            if (strPassword == "" || strPassword02 != strPassword || strPassword.length < 8) {
                document.querySelector("#txtPassword01").value = "";
                document.querySelector("#txtPassword02").value = "";
                swal("Error !!", "La Password ingresada no es valida !!", "error");
                return false;
            } else {
                let formData = new FormData(formResetPass);
                axios.post("/login/setPassword", formData).then(function (response) {
                    if (response.data.success) {
                        swal({
                            title: "Exito !",
